...
create_NAM = function(metadata,
  ...
  ## Obtain a set of nodes from an adjacency matrix by random walk
  ...
  diffuse_step <- function(data, s) {
    a <- data$connectivities  ## 1. Get the connectivity matrix `a` from `data$connectivities`. This matrix represents the connectivity of the graph, where the rows and columns represent the vertices (here cells) of the graph, and the values ​​represent the connection strengths (or weights) between those vertices.
    degrees <- Matrix::colSums(a) + 1 # degrees = number of edges joined to each graph vertex (=each cell) ## 2. Compute the degree of each vertex (that is, the number of edges connected to that vertex). Degree is an indicator of the "importance" of a vertex on a graph. Here we use `Matrix::colSums(a)` to calculate the degree of each vertex and add 1.
    s_norm <- s / degrees ## 3. Normalize the input vector `s` by the degree of each vertex. This will scale each element of the vector `s` based on the "importance" of its vertices.
    res <- (a %*% s_norm) + s_norm  ## 4. Then multiply the connection matrix `a` by the normalized vector `s_norm` and add the original vector `s_norm`. In this calculation, each vertex's new value is the sum of the normalized values ​​of the other vertices connected to it. This simulates the "diffusion" of information.
    return(as.matrix(res)) ## remove sparsity ## 5. Finally, return the result as a dense matrix. This function specifies that the result should be returned as a dense matrix rather than a sparse matrix.
  }
  # These are the main functions of this function `diffuse_step`. A technique commonly found in network data and algorithms based on graph theory that uncovers patterns in data by spreading information across a network.
  ## The reason for adding 1 in 2. is probably to prevent division by 0. Since we are dividing by the degree in the s/degrees line, if the degree of the vertex is 0 (that is, the vertex is isolated), we may end up with an infinity or undefined value. By adding this 1, you can avoid such errors. You may also be able to fine-tune the overall scaling to make the spread effect a little smaller.
  # This code repeats a series of diffusion steps, evaluates the result after each step, and exits the loop when a certain condition is met.
  # We provide mechanisms to stop the dissemination of information once it reaches the appropriate stage. To ensure that the diffusion has progressed sufficiently, the median kurtosis is calculated after each step and checked to see if it has decreased below a certain threshold.
  for (i in seq_len(maxnsteps)) { # 1. Start a loop over the sequence generated by `seq_len(maxnsteps)`. This sequence contains integers from 1 to `maxnsteps`, where `maxnsteps` specifies the maximum number of diffusion steps.
    print(i)
    s <- diffuse_step(data, s) # 2. At each step, first execute `diffuse_step(data, s)` to generate a new `s` (a vector representing the information diffusion situation).
    print(s)
    medkurt <- median(apply(prop.table(s, 2), 1, moments::kurtosis)) # 3. Next, normalize `s` so that each column sums to 1 using `prop.table(s, 2)` and add `moments::kurtosis` for each column. to calculate kurtosis (a statistic that describes the sharpness of the distribution of values). Compute their median kurtosis `medkurt`.
    if (is.null(nsteps)) { # 4. If `is.null(nsteps)` is true, that is, `nsteps` is not specified, then `medkurt` is reduced by 3 or less from the kurtosis of the previous step `prevmedkurt`, or Terminate the loop if it repeats more than 3 times. This is the condition for determining that information has spread sufficiently.
      prevmedkurt <- medkurt
      print(prevmedkurt)
      print(medkurt)
      if (prevmedkurt - medkurt < 3 & i > 3) { # 5. Otherwise (if `nsteps` is specified), exit the loop when the specified number of steps `nsteps` is reached.
...
NAM_NMF_GP = function(seurat_object=NULL,
  ...
  # deltasse <-  ssered - ssefull
  # f <- (deltasse / k) / (ssefull/n) 
  # minp <- -pf(f, k, n-(1+length(covs)+k), log.p = TRUE) 
  ...
  if (local_test) {    
    ...
    Nnull <- min(1000, Nnull)
    y_ <- y_[, 1:Nnull]
    gamma_ <- crossprod(U[, 1:k], y_) # Computing the cross product (or outer product) of the first k columns of the U matrix (i.e. the k principal components) and the y_vector (the sample disease information). The crossprod(x, y) function in R is equivalent to transpose(x) %*% y, which computes the transpose of matrix x and the matrix product of matrix y. This can be interpreted as calculating the correlation between each component value and disease information. This makes it possible to evaluate the extent to which each component value can explain disease information. The resulting gamma_ is a vector of length k, representing the weight of disease information for each principal component. Specifically, gamma_[i] represents the correlation (or covariance) between the i-th principal component and disease information.。
